...
import {combineLatest, forkJoin, mergeMap, Observable, of, Subscription, switchMap, tap} from "rxjs";
...
export class JobOfferApplyComponent implements OnInit {
  ...
  @Select(CandidateState.getCandidate) candidate$!: Observable<CandidateRequest>
  ...
  onSubmit(data: CandidateRequest) {
    this.store.dispatch(new AddCandidate(data, this.jobOfferId))
    .pipe(
      mergeMap(()=>
        combineLatest([
          this.candidate$,
        ...
        this.notificationService.notify(`${candidate.firstName}, aplikowałeś na ofertę ${jobOffer.position}`)
